...
public class SensorProviderListener {
    ...
    public SensorProviderListener(Context context) {
        ...
        if ("N1T".equals(SystemProperties.get("ro.product.device"))) {
            mShakeSpeed = PreferencesSensorProvider.getShakeDegree(context)*72;
        } else {
            mShakeSpeed = PreferencesSensorProvider.getShakeDegree(context)*112;
        }
    ...
    private final SensorEventListener mSmartCallEventListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (event.sensor.getType() == Sensor.TYPE_PROXIMITY) {
                float distance = event.values[0];
                boolean changeFlag = (distance >= 0.0f && distance < PROXIMITY_THRESHOLD && distance < mProximitySensor.getMaximumRange());
                if (changeFlag && getGy > DEFAULT_SMART_CALL_DEGREE) {
